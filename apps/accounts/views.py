...
def get_otp(request):
    ...
    return Response({"message": "Accepted. Check your email"}, status=status.HTTP_202_ACCEPTED)
...
def verify_otp(request):
    ...
    if not validation_result.get("is_valid"):
        create_audit_log.delay(Audit.OTP_FAILED, email, **data_)  # type: ignore
        return Response(
            {"detail": "Invalid or expired OTP."},
            status=status.HTTP_400_BAD_REQUEST,
        )
    user, _ = User.objects.update_or_create(email=email, defaults={})
    create_audit_log.delay(Audit.OTP_VERIFIED, email, **data_)  # type: ignore
